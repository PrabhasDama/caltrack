...
export async function getProgress() {
  const [{ client, user }, dashboard] = await Promise.all([
    requireProfile(),
    getDashboard(),
  ]);
  const from = addDays(dashboard.today, -41),
    today = dashboard.today;
  const [weights, measurements, meals, extras, workouts, purchases, budget] =
    await Promise.all([
      ...
      allRows<NutritionLog>((a, b) =>
        client
          ...
          .select("local_date,status,calories,protein,carbs,fat,fiber")
          .eq("user_id", user.id)
          .gte("local_date", from)
          .lte("local_date", today)
          .order("id")
      ...
      allRows<{ purchased_on: string; total: number; currency: string }>(
        (a, b) =>
          client
            .from("purchases")
  ...
  if (budget.error) throw new Error("Your budget could not be loaded.");
  return {
    ...dashboard,
    weights,
    measurements,
