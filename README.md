...
[Live Demo] [Tests Passing] [Next.js] [TypeScript]
...
A full-stack nutrition and grocery intelligence platform
that connects meal planning, pantry inventory, grocery
shopping, nutrition tracking, and budget optimization.
...
💰 Multi-Store Budget Optimization
📊 Nutrition & Progress Analytics
🧾 Receipt & Purchase Reconciliation
...
The publishable key is designed for app use. **Never add a service-role/secret key to a `NEXT_PUBLIC_` variable.** Kroger catalog imports additionally require a server-only service-role credential; see the provider setup below. `.env.local`, `.env.qa`, build files, and test output are excluded from Git.
...
The connected development project has migrations 001–026 applied. Never use `supabase db reset --linked` against an account with real data.
...
The commands above run the core smoke test. The full twelve-scenario suite requires all three disposable accounts; use the full browser QA setup described under Phase 9 below.
...
**RELEASE CANDIDATE VERIFIED** for controlled tester release of the current core product. All **193 automated tests**, **12 browser scenarios**, typecheck, lint, the production build, and **8 live rollback security suites** pass. The browser scenarios ran in two ordered batches (3 + 9) using fresh disposable accounts. Mobile/tablet layouts, real-location checkout, independent settings, receipt corrections, exactly-once shopping reconciliation, and mixed-price baskets are verified. Temporary QA accounts, uploads and local artifacts were removed. CHECKPOINT.md contains detailed evidence.

Kroger Certification returned real store/product data and a price for the tested priced product; products without prices remain explicitly unavailable. Certification prices are labeled and excluded from automatic quote optimization. Approved production Kroger pricing is unverified; optional live maps and automatic OCR are unconfigured. Manual fallbacks pass. This verification does not deploy the public site or add later-phase features.

## Historical implementation checkpoints

The following milestone notes preserve the results and limitations at the time of each implementation phase. Their test counts and unfinished-phase statements are historical; the latest release verification is recorded in CHECKPOINT.md.
...
## Real locations and Kroger provider

`/stores` provides explicit ZIP/device-location discovery, preferred/excluded locations, product search/details, private food/package review and opt-in sharing of future actual package prices. Settings links here without restarting onboarding. Precise device coordinates are transient and never saved. Maps require a browser-restricted Google Maps JavaScript API key and map ID; list discovery works without a map key. Non-Kroger/Google Places discovery and Open Prices remain unconnected boundaries; no placeholder businesses or price feeds are created.

Configure server-only `KROGER_CLIENT_ID`, `KROGER_CLIENT_SECRET`, `KROGER_ENVIRONMENT` and `SUPABASE_SERVICE_ROLE_KEY` locally and in Vercel. Certification is the default environment; production requires explicitly setting `KROGER_ENVIRONMENT` to `production` and approved credentials. Restrict the public map key to your deployed HTTP referrers and Maps JavaScript API; it is the only provider key intended for browsers. Optional provider absence does not block manual shopping. No actual credential belongs in source control.

Run `npx tsx scripts/kroger-capabilities.ts` for a bounded diagnostic at two Long Beach locations. The sanitized report is `docs/kroger-capabilities.json`. Observed Certification prices are nested in `items[].price.regular`; some products omit price entirely, and the same product may differ between locations. A product search term is not proof of a food match. The diagnostic does not claim a production price capability.

The server adapter uses client-credentials OAuth, token expiry safety margin, deduplicated requests, a bounded 200-entry five-minute response cache, short negative caching, ten-second per-attempt timeout and at most three attempts for transient network/429/5xx failures. Credential/authorization errors are not retried. API actions require authentication and allow twelve requests per user per minute per running server instance. This in-memory limiter/cache is not a global multi-instance quota; set provider-side quotas before wider launch. Diagnostics contain operation/status/count/timing, never credentials, coordinates, search terms or account IDs. External product searches are explicit and bounded, never triggered by ordinary tab navigation.

Migration 025 extends the existing location/product/offer/history tables. Only the server importer can create shared provider quotes. Per-user location choices and reviewed product/food mappings have RLS. Unknown packages retain null weight until explicitly reviewed. Known mass units convert exactly; counts and volume never imply density. Saving a food match does not change a meal or grocery requirement.

Provider quotes are Current up to 60 minutes, Recent up to 24 hours, then outdated; the two server threshold variables allow UI/eligibility tuning, while database acceptance retains a conservative 24-hour ceiling. Certification, missing, unavailable and stale quotes cannot be applied as provider prices. Actual private purchases remain separately labeled last-confirmed observations. Unknown prices never become zero. Development/explicit local QA may use labeled demo fixtures; Vercel production never enables them. Groceries and Best Split use the same eligible normalized offers.

Confirmed package purchases reuse private receipt observations. Shared observations are opt-in, contain only product/location/price/package/date/source, and are corrected/reversed with the original ledger exactly once. No identity or receipt linkage is exposed in the shared table; date/location still carry residual reidentification risk, so these records are data-minimized, not a claim of irreversible anonymity. Turning sharing off removes contributions. Shared observations are not automatically trusted as optimizer quotes; private confirmed purchases already feed their owner's optimizer. Open Prices licensing and identity mapping must be reviewed before enabling a shared import.

Provider documentation: [Kroger API reference](https://developer.kroger.com/reference), [Google advanced markers](https://developers.google.com/maps/documentation/javascript/advanced-markers/add-marker), [Google Places storage/attribution rules](https://developers.google.com/maps/documentation/places/web-service/policies). No Google Places response database is created.
