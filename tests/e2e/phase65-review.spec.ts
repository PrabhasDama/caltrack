...
test("measurement preferences, recipe review, pantry selection, and mobile dialogs", async ({
  ...
  expect(await recipe.locator("ol li").count()).toBeGreaterThanOrEqual(3);
  ...
  await recipe.getByRole("button", { name: "Swap meal", exact: true }).click();
  await page
  ...
  await page.screenshot({ path: "test-results/review-preferences-mobile.png" });
  await page
    .getByRole("button", { name: "Save country & measurement", exact: true })
    .click();
  await expect(page.getByRole("dialog")).toHaveCount(0);
  ...
  await page
    ...
    .click();
  await page.screenshot({ path: "test-results/review-correction-mobile.png" });
  await page.getByRole("button", { name: "Close dialog", exact: true }).click();
